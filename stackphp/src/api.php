...
class API
{
    ...
    /**
      ...
      */
    public static $key = 'T9t9ERWmWNgxItGKtXQq2A((';
    ...
    /**
      * This value is used by the code that makes HTTP requests. It is recommended that
      * your application explicitly modify this value for identification purposes.
      */
    public static $user_agent = 'Stack.PHP (v0.5 beta 1)';
    ...
    /**
      * Note: this variable should not be used to enable debug mode. Instead,
      * use API::EnableDebugMode to toggle debug mode.
      */
    ...
    public static function EnableDebugMode($enable_debug=TRUE)
    {
        ...
        // Enable the debug filter (which includes 'type')
        Filter::$default_filter            = '!-q2RjmHZ';
        Filter::$default_pagination_filter = '!-q2RjmDn';
    }
    ...
    /// Returns the current version of Stack.PHP
    /**
      * \return the current version of Stack.PHP
     */
    ...
    /**
      ...
      * \param $include_total whether or not the total number of items is needed in the response
      ...
      */
    public static function GetJSON($url, $include_total=FALSE)
    {
        ...
        if(self::$cache !== null && $data = self::$cache->RetrieveFromCache($url->CompleteURL(FALSE, $include_total)))
        {
            if(self::$debug)
                echo "[DEBUG]: Found '" . $url->CompleteURL(FALSE, $include_total) . "' in cache.\n";
        ...
        if(self::$debug)
        {
            if($url->GetPOSTData() !== null)
            {
                echo "[DEBUG]: Issuing POST request '" . $url->CompleteURL(TRUE, $include_total) . "'\n";
                foreach($url->GetPOSTData() as $key=>$value)
                    echo "  $key => '$value'\n";
            }
            else
                echo "[DEBUG]: Issuing GET request '" . $url->CompleteURL(TRUE, $include_total) . "'\n";
        }
        ...
        // Retrieve the data
        $data = self::GetRawData($url->CompleteURL(TRUE, $include_total), $url->GetPOSTData());
        ...
        // The data is valid, so add it to the cache if applicable using the provided TTL
        if(self::$cache !== null && $url->GetCacheTTL())
            self::$cache->AddToCache($url->CompleteURL(FALSE, $include_total), serialize($data), $url->GetCacheTTL());
    ...
    {
        return new UserRequest(new URL(), $access_token);
    }
